defmodule ColorMatching.PaletteStorageTest do
  ...
  describe "validate_palette_name/1" do
    ...
    test "rejects names that are too long" do
      ...
      assert {:error, "Name too long (max 50 characters)"} =
               PaletteStorage.validate_palette_name(long_name)
    end

    test "rejects names that conflict with presets" do
      assert {:error, "Name conflicts with preset palette"} =
               PaletteStorage.validate_palette_name("Warm")

      assert {:error, "Name conflicts with preset palette"} =
               PaletteStorage.validate_palette_name("Cool")

      assert {:error, "Name conflicts with preset palette"} =
               PaletteStorage.validate_palette_name("Neon")
    end
